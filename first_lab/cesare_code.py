...
def choose_sequence(symbol: str) -> tuple:
    """
    Simple choosing between ascii symbols and russian symbols
    :param symbol: str
...
def vigener_cipher(text: str, secret_key: str, *, encrypt: bool = True) -> str:
    """
    Encrypt/decrypt text using Vigener cipher.
    encrypt = True -> encrypting, encrypt = False -> decrypting
    :param text: str
    ...
    for index, symbol in enumerate(text):
        ...
        if symbol.islower():
            text[index] = lower_letters[
                operation(lower_letters.index(symbol), lower_letters.index(secret_key[additional_value]))
                % len(lower_letters)]
        elif symbol.isupper():
            text[index] = upper_letters[
